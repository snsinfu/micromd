...
TEST_CASE("neighbor_searcher_v2 - outputs nothing by default")
{
    md::neighbor_searcher_v2<md::open_box> searcher{md::open_box{}, 1};
    std::vector<std::pair<md::index, md::index>> pairs;
    searcher.search(std::back_inserter(pairs));
    CHECK(pairs.empty());
}

TEST_CASE("neighbor_searcher_v2 - accepts empty point cloud")
{
    md::neighbor_searcher_v2<md::open_box> searcher{md::open_box{}, 1};
    std::vector<std::pair<md::index, md::index>> pairs;
...
TEST_CASE("neighbor_searcher_v2 - finds correct neighbors in point cloud")
{
    ...
    auto compute_actual_expect = [&](auto box) {
        ...
        // Actual output from neighbor_searcher_v2.
        using box_type = decltype(box);
        md::neighbor_searcher_v2<box_type> searcher{box, neighbor_distance};
        searcher.set_points(points);
...
TEST_CASE("neighbor_searcher_v2 - finds correct neighbors of query point")
{
    md::index const point_count = 100;
    md::scalar const neighbor_distance = 0.3;
